...
class Topics extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {expanded: false, search: '', selected: store.get('selected_tags') || []};
    }
    ...
    render() {
        const {
            ...
            state: {expanded, search, selected},
            onChangeSearch, onSelectTag, expand
        } = this;
        ...
        const selectedKeys = selected.map(key => {return <li className="Topics__selected__tag" key={`selected-${key}`}><a className="action" onClick={() => onSelectTag(key)}>×</a><a className="tagname" onClick={() => onSelectTag(key)}>{detransliterate(key)}</a></li> })
        ...
        return (
            <ul className={cn}>
               ...
               {categories}
               <li className="Topics__title" key="mytags">Фильтрация по тегам</li>
               {selectedKeys.length ? selectedKeys : <li className="Topics__selected__tag"><span>не выбрано ни одного тега</span></li>}
